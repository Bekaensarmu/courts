 ...
 * Page class
 */
class Report6Summary extends Report6
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "Report6Summary";
    ...
    public $CurrentPageName = "Report6";
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["Report6"]) || $GLOBALS["Report6"]::class == PROJECT_NAMESPACE . "Report6") {
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Report6');
        }
    ...
    public function loadGroupRowValues()
    {
        ...
        if ($this->GroupCount < $cnt) {
            $this->Name->setGroupValue(reset($this->GroupRecords[$this->GroupCount]));
        } else {
            $this->Name->setGroupValue("");
        }
    }
    ...
    public function loadRowValues($record)
    {
        $data = [];
        $data["id"] = $record['id'];
        $data["attorneyID"] = $record['attorneyID'];
        $data["Name"] = $record['Name'];
        $data["phone"] = $record['phone'];
        $data["Address"] = $record['Address'];
        $data["State"] = $record['State'];
        $data["EmpType"] = $record['EmpType'];
        ...
        $this->attorneyID->setDbValue($record['attorneyID']);
        $this->Name->setDbValue(GroupValue($this->Name, $record['Name']));
        $this->phone->setDbValue($record['phone']);
        $this->Address->setDbValue($record['Address']);
        $this->State->setDbValue($record['State']);
        $this->EmpType->setDbValue($record['EmpType']);
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::SEARCH) {
            ...
            if ($this->RowTotalType == RowSummary::GROUP) {
                $this->RowAttrs["data-group"] = $this->Name->groupValue(); // Set up group attribute
            }

            // Name
            $this->Name->GroupViewValue = $this->Name->groupValue();
            $this->Name->CellCssClass = ($this->RowGroupLevel == 1 ? "ew-rpt-grp-summary-1" : "ew-rpt-grp-field-1");
            $this->Name->GroupViewValue = DisplayGroupValue($this->Name, $this->Name->GroupViewValue);
            ...
            // Name
            $this->Name->GroupViewValue = $this->Name->groupValue();
            $this->Name->CellCssClass = "ew-rpt-grp-field-1";
            $this->Name->GroupViewValue = DisplayGroupValue($this->Name, $this->Name->GroupViewValue);
            if (!$this->Name->LevelBreak) {
                $this->Name->GroupViewValue = "";
            } else {
                $this->Name->LevelBreak = false;
            }
            ...
            $this->Address->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
        ...
        if ($this->RowType == RowType::TOTAL) {
            // Name
            $currentValue = $this->Name->GroupViewValue;
            $viewValue = &$this->Name->GroupViewValue;
            ...
            // Name
            $currentValue = $this->Name->groupValue();
            $viewValue = &$this->Name->GroupViewValue;
            $viewAttrs = &$this->Name->ViewAttrs;
            $cellAttrs = &$this->Name->CellAttrs;
            $hrefValue = &$this->Name->HrefValue;
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fReport6srch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->Name->Visible || $this->phone->Visible;
    }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fReport6srch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fReport6srch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    protected function getExtendedFilter()
    {
        ...
        if ($restoreSession) {
            $restoreDefault = true;
            if ($this->Name->AdvancedSearch->issetSession()) { // Field Name
                $this->Name->AdvancedSearch->load();
                $restoreDefault = false;
            }
            if ($this->phone->AdvancedSearch->issetSession()) { // Field phone
        ...
        $this->buildExtendedFilter($this->phone, $filter, false, true); // Field phone
    ...
    {
        // Field Name
        $this->Name->AdvancedSearch->loadDefault();

        // Field phone
        $this->phone->AdvancedSearch->loadDefault();
    }
    ...
    public function showFilterList()
    {
        ...
        // Field phone
        $extWrk = "";
        $this->buildExtendedFilter($this->phone, $extWrk);
        $filter = "";
        if ($extWrk != "") {
            $filter .= "<span class=\"ew-filter-value\">$extWrk</span>";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fReport6srch");
        }
        ...
        // Field phone
        $wrk = "";
        if ($this->phone->AdvancedSearch->SearchValue != "" || $this->phone->AdvancedSearch->SearchValue2 != "") {
            $wrk = "\"x_phone\":\"" . JsEncode($this->phone->AdvancedSearch->SearchValue) . "\"," .
                "\"z_phone\":\"" . JsEncode($this->phone->AdvancedSearch->SearchOperator) . "\"," .
                "\"v_phone\":\"" . JsEncode($this->phone->AdvancedSearch->SearchCondition) . "\"," .
                "\"y_phone\":\"" . JsEncode($this->phone->AdvancedSearch->SearchValue2) . "\"," .
                "\"w_phone\":\"" . JsEncode($this->phone->AdvancedSearch->SearchOperator2) . "\"";
        }
        if ($wrk != "") {
            if ($filterList != "") {
                $filterList .= ",";
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fReport6srch", $filters);
    ...
    protected function setupFilterList($filter)
    {
        ...
        // Field Name
        if (!$this->Name->AdvancedSearch->get($filter)) {
            $this->Name->AdvancedSearch->loadDefault(); // Clear filter
        }
        $this->Name->AdvancedSearch->save();

        // Field phone
        if (!$this->phone->AdvancedSearch->get($filter)) {
            $this->phone->AdvancedSearch->loadDefault(); // Clear filter
        }
        $this->phone->AdvancedSearch->save();
        return true;
    }
