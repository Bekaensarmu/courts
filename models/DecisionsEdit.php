 ...
 * Page class
 */
class DecisionsEdit extends Decisions
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DecisionsEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "DecisionsEdit";
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("DecisionsList"); // Return to list page
                        return;
    ...
    protected function getEditRow($rsold)
    {
        ...
        // decisionDate
        $this->decisionDate->setDbValueDef($rsnew, UnFormatDateTime($this->decisionDate->CurrentValue, $this->decisionDate->formatPattern()), $this->decisionDate->ReadOnly);

        // Decisiontype
        $this->Decisiontype->setDbValueDef($rsnew, $this->Decisiontype->CurrentValue, $this->Decisiontype->ReadOnly);

        // kistype
        $this->kistype->setDbValueDef($rsnew, $this->kistype->CurrentValue, $this->kistype->ReadOnly);

        // ez
        $this->ez->setDbValueDef($rsnew, $this->ez->CurrentValue, $this->ez->ReadOnly);

        // chilot
        $this->chilot->setDbValueDef($rsnew, $this->chilot->CurrentValue, $this->chilot->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        if (isset($row['decisionDate'])) { // decisionDate
            $this->decisionDate->CurrentValue = $row['decisionDate'];
        }
        if (isset($row['Decisiontype'])) { // Decisiontype
            $this->Decisiontype->CurrentValue = $row['Decisiontype'];
        }
        if (isset($row['kistype'])) { // kistype
            $this->kistype->CurrentValue = $row['kistype'];
        }
        if (isset($row['ez'])) { // ez
            $this->ez->CurrentValue = $row['ez'];
        }
        if (isset($row['chilot'])) { // chilot
            $this->chilot->CurrentValue = $row['chilot'];
        }
