 ...
 * Page class
 */
class Report5Summary extends Report5
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "Report5Summary";
    ...
    public $CurrentPageName = "Report5";
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["Report5"]) || $GLOBALS["Report5"]::class == PROJECT_NAMESPACE . "Report5") {
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Report5');
        }
    ...
    public function loadGroupRowValues()
    {
        ...
        if ($this->GroupCount < $cnt) {
            $this->decisionDate->setGroupValue(reset($this->GroupRecords[$this->GroupCount]));
        } else {
            $this->decisionDate->setGroupValue("");
        }
    }
    ...
    public function loadRowValues($record)
    {
        $data = [];
        $data["id"] = $record['id'];
        $data["decisionDate"] = $record['decisionDate'];
        $data["Decisiontype"] = $record['Decisiontype'];
        $data["Description"] = $record['Description'];
        $data["kistype"] = $record['kistype'];
        $data["ez"] = $record['ez'];
        $data["chilot"] = $record['chilot'];
        ...
        $this->decisionDate->setDbValue(GroupValue($this->decisionDate, $record['decisionDate']));
        $this->Decisiontype->setDbValue($record['Decisiontype']);
        $this->Description->setDbValue($record['Description']);
        $this->kistype->setDbValue($record['kistype']);
        $this->ez->setDbValue($record['ez']);
        $this->chilot->setDbValue($record['chilot']);
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::SEARCH) {
            ...
            $this->decisionDate->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->decisionDate->AdvancedSearch->SearchValue, $this->decisionDate->formatPattern()), $this->decisionDate->formatPattern()));
            $this->decisionDate->PlaceHolder = RemoveHtml($this->decisionDate->caption());
            $this->decisionDate->setupEditAttributes();
            $this->decisionDate->EditValue2 = HtmlEncode(FormatDateTime(UnFormatDateTime($this->decisionDate->AdvancedSearch->SearchValue2, $this->decisionDate->formatPattern()), $this->decisionDate->formatPattern()));
            $this->decisionDate->PlaceHolder = RemoveHtml($this->decisionDate->caption());
            ...
            if ($this->RowTotalType == RowSummary::GROUP) {
                $this->RowAttrs["data-group"] = $this->decisionDate->groupValue(); // Set up group attribute
            }

            // decisionDate
            $this->decisionDate->GroupViewValue = $this->decisionDate->groupValue();
            $this->decisionDate->GroupViewValue = FormatDateTime($this->decisionDate->GroupViewValue, $this->decisionDate->formatPattern());
            $this->decisionDate->CellCssClass = ($this->RowGroupLevel == 1 ? "ew-rpt-grp-summary-1" : "ew-rpt-grp-field-1");
            $this->decisionDate->GroupViewValue = DisplayGroupValue($this->decisionDate, $this->decisionDate->GroupViewValue);
            ...
            $this->decisionDate->GroupViewValue = $this->decisionDate->groupValue();
            $this->decisionDate->GroupViewValue = FormatDateTime($this->decisionDate->GroupViewValue, $this->decisionDate->formatPattern());
            $this->decisionDate->CellCssClass = "ew-rpt-grp-field-1";
            $this->decisionDate->GroupViewValue = DisplayGroupValue($this->decisionDate, $this->decisionDate->GroupViewValue);
            if (!$this->decisionDate->LevelBreak) {
                $this->decisionDate->GroupViewValue = "";
            } else {
                $this->decisionDate->LevelBreak = false;
            }
            ...
            $this->Decisiontype->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
        ...
        if ($this->RowType == RowType::TOTAL) {
            // decisionDate
            $currentValue = $this->decisionDate->GroupViewValue;
            $viewValue = &$this->decisionDate->GroupViewValue;
            $viewAttrs = &$this->decisionDate->ViewAttrs;
            $cellAttrs = &$this->decisionDate->CellAttrs;
            $hrefValue = &$this->decisionDate->HrefValue;
            $linkAttrs = &$this->decisionDate->LinkAttrs;
            $this->cellRendered($this->decisionDate, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
        } else {
            // decisionDate
            $currentValue = $this->decisionDate->groupValue();
            $viewValue = &$this->decisionDate->GroupViewValue;
            $viewAttrs = &$this->decisionDate->ViewAttrs;
            $cellAttrs = &$this->decisionDate->CellAttrs;
            $hrefValue = &$this->decisionDate->HrefValue;
            ...
            // Decisiontype
            $currentValue = $this->Decisiontype->CurrentValue;
            $viewValue = &$this->Decisiontype->ViewValue;
            $viewAttrs = &$this->Decisiontype->ViewAttrs;
            $cellAttrs = &$this->Decisiontype->CellAttrs;
            $hrefValue = &$this->Decisiontype->HrefValue;
            $linkAttrs = &$this->Decisiontype->LinkAttrs;
            $this->cellRendered($this->Decisiontype, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
    ...
    protected function setupFieldCount()
    {
        ...
        if ($this->decisionDate->Visible) {
            $this->GroupColumnCount += 1;
        }
        if ($this->Decisiontype->Visible) {
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fReport5srch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->decisionDate->Visible;
    }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fReport5srch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fReport5srch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    protected function getExtendedFilter()
    {
        ...
        if (Get("cmd") == "reset") {
            // Set default values
            $this->decisionDate->AdvancedSearch->unsetSession();
            $restoreDefault = true;
        } else {
            $restoreSession = !$this->SearchCommand;

            // Field decisionDate
            if ($this->decisionDate->AdvancedSearch->get()) {
            }
        ...
        if ($restoreSession) {
            $restoreDefault = true;
            if ($this->decisionDate->AdvancedSearch->issetSession()) { // Field decisionDate
                $this->decisionDate->AdvancedSearch->load();
                $restoreDefault = false;
            }
        }
        ...
        $this->buildExtendedFilter($this->decisionDate, $filter, false, true); // Field decisionDate
    ...
    {
        // Field decisionDate
        $this->decisionDate->AdvancedSearch->loadDefault();
    }
    ...
    public function showFilterList()
    {
        ...
        // Field decisionDate
        $extWrk = "";
        $this->buildExtendedFilter($this->decisionDate, $extWrk);
        $filter = "";
        if ($extWrk != "") {
            $filter .= "<span class=\"ew-filter-value\">$extWrk</span>";
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->decisionDate->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fReport5srch");
        }

        // Field decisionDate
        $wrk = "";
        if ($this->decisionDate->AdvancedSearch->SearchValue != "" || $this->decisionDate->AdvancedSearch->SearchValue2 != "") {
            $wrk = "\"x_decisionDate\":\"" . JsEncode($this->decisionDate->AdvancedSearch->SearchValue) . "\"," .
                "\"z_decisionDate\":\"" . JsEncode($this->decisionDate->AdvancedSearch->SearchOperator) . "\"," .
                "\"v_decisionDate\":\"" . JsEncode($this->decisionDate->AdvancedSearch->SearchCondition) . "\"," .
                "\"y_decisionDate\":\"" . JsEncode($this->decisionDate->AdvancedSearch->SearchValue2) . "\"," .
                "\"w_decisionDate\":\"" . JsEncode($this->decisionDate->AdvancedSearch->SearchOperator2) . "\"";
        }
        if ($wrk != "") {
            if ($filterList != "") {
                $filterList .= ",";
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fReport5srch", $filters);
    ...
    protected function setupFilterList($filter)
    {
        ...
        // Field decisionDate
        if (!$this->decisionDate->AdvancedSearch->get($filter)) {
            $this->decisionDate->AdvancedSearch->loadDefault(); // Clear filter
        }
        $this->decisionDate->AdvancedSearch->save();
        return true;
    }
