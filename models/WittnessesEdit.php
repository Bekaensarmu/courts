 ...
 * Page class
 */
class WittnessesEdit extends Wittnesses
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "WittnessesEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "WittnessesEdit";
    ...
    public function run()
    {
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("WittnessesList"); // Return to list page
                        return;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("WittnessesList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "WittnessesList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "WittnessesList") {
                            ...
                            $returnUrl = "WittnessesList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'wittnessID' first before field var 'x_wittnessID'
        $val = $CurrentForm->hasValue("wittnessID") ? $CurrentForm->getValue("wittnessID") : $CurrentForm->getValue("x_wittnessID");
        if (!$this->wittnessID->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->wittnessID->Visible = false; // Disable update for API request
            } else {
                $this->wittnessID->setFormValue($val);
            }
        }
        ...
        // Check field name 'wittnessType' first before field var 'x_wittnessType'
        $val = $CurrentForm->hasValue("wittnessType") ? $CurrentForm->getValue("wittnessType") : $CurrentForm->getValue("x_wittnessType");
        if (!$this->wittnessType->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->wittnessType->Visible = false; // Disable update for API request
            } else {
                $this->wittnessType->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->wittnessID->CurrentValue = $this->wittnessID->FormValue;
        $this->name->CurrentValue = $this->name->FormValue;
        $this->address->CurrentValue = $this->address->FormValue;
        $this->state->CurrentValue = $this->state->FormValue;
        $this->wittnessType->CurrentValue = $this->wittnessType->FormValue;
    ...
    public function renderRow()
    {
        ...
        // wittnessID
        $this->wittnessID->RowCssClass = "row";
        ...
        // wittnessType
        $this->wittnessType->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // wittnessID
            $this->wittnessID->ViewValue = $this->wittnessID->CurrentValue;
            ...
            // wittnessID
            $this->wittnessID->setupEditAttributes();
            if (!$this->wittnessID->Raw) {
                $this->wittnessID->CurrentValue = HtmlDecode($this->wittnessID->CurrentValue);
            }
            $this->wittnessID->EditValue = HtmlEncode($this->wittnessID->CurrentValue);
            $this->wittnessID->PlaceHolder = RemoveHtml($this->wittnessID->caption());
            ...
            // wittnessType
            $this->wittnessType->EditValue = $this->wittnessType->options(false);
            $this->wittnessType->PlaceHolder = RemoveHtml($this->wittnessType->caption());
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->wittnessID->Visible && $this->wittnessID->Required) {
                if (!$this->wittnessID->IsDetailKey && EmptyValue($this->wittnessID->FormValue)) {
                    $this->wittnessID->addErrorMessage(str_replace("%s", $this->wittnessID->caption(), $this->wittnessID->RequiredErrorMessage));
                }
            }
            ...
            if ($this->wittnessType->Visible && $this->wittnessType->Required) {
                if ($this->wittnessType->FormValue == "") {
                    $this->wittnessType->addErrorMessage(str_replace("%s", $this->wittnessType->caption(), $this->wittnessType->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // wittnessID
        $this->wittnessID->setDbValueDef($rsnew, $this->wittnessID->CurrentValue, $this->wittnessID->ReadOnly);
        ...
        // wittnessType
        $this->wittnessType->setDbValueDef($rsnew, $this->wittnessType->CurrentValue, $this->wittnessType->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['wittnessID'])) { // wittnessID
            $this->wittnessID->CurrentValue = $row['wittnessID'];
        }
        ...
        if (isset($row['wittnessType'])) { // wittnessType
            $this->wittnessType->CurrentValue = $row['wittnessType'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("WittnessesList"), "", $this->TableVar, true);
        $pageId = "edit";
