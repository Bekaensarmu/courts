...
/**
 * Table class for decisions
 */
class Decisions extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->decisionDate->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['decisionDate'] = &$this->decisionDate;

        // Decisiontype
        $this->Decisiontype = new DbField(
            $this, // Table
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->decisionDate->DbValue = $row['decisionDate'];
        $this->Decisiontype->DbValue = $row['Decisiontype'];
        $this->kistype->DbValue = $row['kistype'];
        $this->ez->DbValue = $row['ez'];
        $this->chilot->DbValue = $row['chilot'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("DecisionsList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "DecisionsView" => $Language->phrase("View"),
            "DecisionsEdit" => $Language->phrase("Edit"),
            "DecisionsAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "DecisionsView",
            Config("API_ADD_ACTION") => "DecisionsAdd",
            Config("API_EDIT_ACTION") => "DecisionsEdit",
            Config("API_DELETE_ACTION") => "DecisionsDelete",
            Config("API_LIST_ACTION") => "DecisionsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("DecisionsView", $parm);
        } else {
            $url = $this->keyUrl("DecisionsView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("DecisionsEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("DecisionsList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("DecisionsAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("DecisionsDelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->decisionDate->EditValue = FormatDateTime($this->decisionDate->CurrentValue, $this->decisionDate->formatPattern());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->decisionDate);
                    $doc->exportCaption($this->Decisiontype);
                    $doc->exportCaption($this->kistype);
                    $doc->exportCaption($this->ez);
                    $doc->exportCaption($this->chilot);
        ...
        while (($row = $result->fetch()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->decisionDate);
                        $doc->exportField($this->Decisiontype);
                        $doc->exportField($this->kistype);
                        $doc->exportField($this->ez);
                        $doc->exportField($this->chilot);
