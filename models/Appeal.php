...
/**
 * Table class for appeal
 */
class Appeal extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->appealDate->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        $this->appealask->Lookup = new Lookup($this->appealask, 'appeal', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->appealask->OptionCount = 4;
        $this->appealask->SearchOperators = ["=", "<>"];
        $this->Fields['appealask'] = &$this->appealask;

        // AppealDescription
        $this->AppealDescription = new DbField(
            $this, // Table
        ...
        $this->AppealDescription->Lookup = new Lookup($this->AppealDescription, 'appeal', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        ...
        // appealDecision
        $this->appealDecision = new DbField(
            $this, // Table
        ...
        $this->appealDecision->Lookup = new Lookup($this->appealDecision, 'appeal', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->AppealID->DbValue = $row['AppealID'];
        $this->appealDate->DbValue = $row['appealDate'];
        $this->mid->DbValue = $row['mid'];
        $this->rank->DbValue = $row['rank'];
        $this->name->DbValue = $row['name'];
        $this->deptName->DbValue = $row['deptName'];
        $this->halafinet->DbValue = $row['halafinet'];
        $this->crimstate->DbValue = $row['crimstate'];
        $this->Description->DbValue = $row['Description'];
        $this->midib->DbValue = $row['midib'];
        $this->appealask->DbValue = $row['appealask'];
        $this->AppealDescription->DbValue = $row['AppealDescription'];
        $this->appealDecision->DbValue = $row['appealDecision'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("AppealList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "AppealView" => $Language->phrase("View"),
            "AppealEdit" => $Language->phrase("Edit"),
            "AppealAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "AppealView",
            Config("API_ADD_ACTION") => "AppealAdd",
            Config("API_EDIT_ACTION") => "AppealEdit",
            Config("API_DELETE_ACTION") => "AppealDelete",
            Config("API_LIST_ACTION") => "AppealList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("AppealView", $parm);
        } else {
            $url = $this->keyUrl("AppealView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AppealEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AppealList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AppealAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AppealList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("AppealDelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->appealDate->EditValue = FormatDateTime($this->appealDate->CurrentValue, $this->appealDate->formatPattern());
        ...
        $this->halafinet->EditValue = $this->halafinet->CurrentValue;
        ...
        $this->crimstate->EditValue = $this->crimstate->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->appealDate);
                    $doc->exportCaption($this->mid);
                    $doc->exportCaption($this->rank);
                    $doc->exportCaption($this->name);
                    $doc->exportCaption($this->deptName);
                    $doc->exportCaption($this->halafinet);
                    $doc->exportCaption($this->crimstate);
                    $doc->exportCaption($this->Description);
                    $doc->exportCaption($this->midib);
                    $doc->exportCaption($this->appealask);
                    $doc->exportCaption($this->AppealDescription);
                    $doc->exportCaption($this->appealDecision);
                    $doc->exportCaption($this->created_at);
                    $doc->exportCaption($this->crimeDate);
                    $doc->exportCaption($this->updated_at);
                    $doc->exportCaption($this->updated_by);
                } else {
                    $doc->exportCaption($this->AppealID);
                    $doc->exportCaption($this->appealDate);
                    $doc->exportCaption($this->mid);
                    $doc->exportCaption($this->rank);
                    $doc->exportCaption($this->name);
        ...
        while (($row = $result->fetch()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->appealDate);
                        $doc->exportField($this->mid);
                        $doc->exportField($this->rank);
                        $doc->exportField($this->name);
                        $doc->exportField($this->deptName);
                        $doc->exportField($this->halafinet);
                        $doc->exportField($this->crimstate);
                        $doc->exportField($this->Description);
                        $doc->exportField($this->midib);
                        $doc->exportField($this->appealask);
                        $doc->exportField($this->AppealDescription);
                        $doc->exportField($this->appealDecision);
                        $doc->exportField($this->created_at);
                        $doc->exportField($this->crimeDate);
                        $doc->exportField($this->updated_at);
                        $doc->exportField($this->updated_by);
                    } else {
                        $doc->exportField($this->AppealID);
                        $doc->exportField($this->appealDate);
                        $doc->exportField($this->mid);
                        $doc->exportField($this->rank);
                        $doc->exportField($this->name);
