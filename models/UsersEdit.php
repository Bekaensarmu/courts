...
class UsersEdit extends Users
{
    ...
    public function setVisibility()
    {
        ...
        $this->current_team_id->setVisibility();
        $this->profile_photo_path->setVisibility();
    ...
    public function run()
    {
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->current_team_id);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'current_team_id' first before field var 'x_current_team_id'
        $val = $CurrentForm->hasValue("current_team_id") ? $CurrentForm->getValue("current_team_id") : $CurrentForm->getValue("x_current_team_id");
        if (!$this->current_team_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->current_team_id->Visible = false; // Disable update for API request
            } else {
                $this->current_team_id->setFormValue($val);
            }
        }

        // Check field name 'profile_photo_path' first before field var 'x_profile_photo_path'
        $val = $CurrentForm->hasValue("profile_photo_path") ? $CurrentForm->getValue("profile_photo_path") : $CurrentForm->getValue("x_profile_photo_path");
        if (!$this->profile_photo_path->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->profile_photo_path->Visible = false; // Disable update for API request
            } else {
                $this->profile_photo_path->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->current_team_id->CurrentValue = $this->current_team_id->FormValue;
        $this->profile_photo_path->CurrentValue = $this->profile_photo_path->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // profile_photo_path
            $this->profile_photo_path->ViewValue = $this->profile_photo_path->CurrentValue;
            ...
            // current_team_id
            $this->current_team_id->HrefValue = "";

            // profile_photo_path
            $this->profile_photo_path->HrefValue = "";
            ...
            if (!$Security->isAdmin() && $Security->isLoggedIn()) { // Non system admin
                ...
                $this->_email->CurrentValue = EmptyValue($this->_email->CurrentValue) ? CurrentUserID() : $this->_email->CurrentValue;
            } else {
                if (!$this->_email->Raw) {
            ...
            // current_team_id
            $this->current_team_id->setupEditAttributes();
            if (!$Security->canAdmin()) { // System admin
                $this->current_team_id->EditValue = $Language->phrase("PasswordMask");
            } else {
                $curVal = trim(strval($this->current_team_id->CurrentValue));
                if ($curVal != "") {
                    $this->current_team_id->ViewValue = $this->current_team_id->lookupCacheOption($curVal);
                } else {
                    $this->current_team_id->ViewValue = $this->current_team_id->Lookup !== null && is_array($this->current_team_id->lookupOptions()) && count($this->current_team_id->lookupOptions()) > 0 ? $curVal : null;
                }
                if ($this->current_team_id->ViewValue !== null) { // Load from cache
                    $this->current_team_id->EditValue = array_values($this->current_team_id->lookupOptions());
                } else { // Lookup from database
                    if ($curVal == "") {
                        $filterWrk = "0=1";
                    } else {
                        $filterWrk = SearchFilter($this->current_team_id->Lookup->getTable()->Fields["userlevelid"]->searchExpression(), "=", $this->current_team_id->CurrentValue, $this->current_team_id->Lookup->getTable()->Fields["userlevelid"]->searchDataType(), "");
                    }
                    $sqlWrk = $this->current_team_id->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    $config->setResultCache($this->Cache);
                    $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                    $ari = count($rswrk);
                    $arwrk = $rswrk;
                    $this->current_team_id->EditValue = $arwrk;
                }
                $this->current_team_id->PlaceHolder = RemoveHtml($this->current_team_id->caption());
            }

            // profile_photo_path
            $this->profile_photo_path->setupEditAttributes();
            $this->profile_photo_path->EditValue = HtmlEncode($this->profile_photo_path->CurrentValue);
            $this->profile_photo_path->PlaceHolder = RemoveHtml($this->profile_photo_path->caption());
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->current_team_id->Visible && $this->current_team_id->Required) {
                if ($Security->canAdmin() && !$this->current_team_id->IsDetailKey && EmptyValue($this->current_team_id->FormValue)) {
                    $this->current_team_id->addErrorMessage(str_replace("%s", $this->current_team_id->caption(), $this->current_team_id->RequiredErrorMessage));
                }
            }
            if ($this->profile_photo_path->Visible && $this->profile_photo_path->Required) {
                if (!$this->profile_photo_path->IsDetailKey && EmptyValue($this->profile_photo_path->FormValue)) {
                    $this->profile_photo_path->addErrorMessage(str_replace("%s", $this->profile_photo_path->caption(), $this->profile_photo_path->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // current_team_id
        if ($Security->canAdmin()) { // System admin
            $this->current_team_id->setDbValueDef($rsnew, $this->current_team_id->CurrentValue, $this->current_team_id->ReadOnly);
        }

        // profile_photo_path
        $this->profile_photo_path->setDbValueDef($rsnew, $this->profile_photo_path->CurrentValue, $this->profile_photo_path->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['current_team_id'])) { // current_team_id
            $this->current_team_id->CurrentValue = $row['current_team_id'];
        }
        if (isset($row['profile_photo_path'])) { // profile_photo_path
            $this->profile_photo_path->CurrentValue = $row['profile_photo_path'];
        }
