 ...
 * Page class
 */
class Wesane2Crosstab extends Wesane2
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "Wesane2Crosstab";
    ...
    public $CurrentPageName = "Wesane2";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'wesane2';
        $this->TableName = 'wesane2';
        ...
        // Table object (wesane2)
        if (!isset($GLOBALS["wesane2"]) || $GLOBALS["wesane2"]::class == PROJECT_NAMESPACE . "wesane2") {
            $GLOBALS["wesane2"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'wesane2');
        }
    ...
    public function loadGroupRowValues()
    {
        ...
        if ($this->GroupCount < $cnt) {
            $this->updated_at->setGroupValue(reset($this->GroupRecords[$this->GroupCount]));
        } else {
            $this->updated_at->setGroupValue("");
        }
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::SEARCH) { // Search row
            ...
            // updated_at
            $this->updated_at->GroupViewValue = $this->updated_at->groupValue();
            $this->updated_at->GroupViewValue = FormatDateTime($this->updated_at->GroupViewValue, $this->updated_at->formatPattern());
            $this->updated_at->CellCssClass = ($this->RowGroupLevel == 1 ? "ew-rpt-grp-summary-1" : "ew-rpt-grp-field-1");
            ...
            $this->updated_at->GroupViewValue = $this->updated_at->groupValue();
            $this->updated_at->GroupViewValue = FormatDateTime($this->updated_at->GroupViewValue, $this->updated_at->formatPattern());
            $this->updated_at->CellCssClass = "ew-rpt-grp-field-1";
            if (!$this->updated_at->LevelBreak) {
                $this->updated_at->GroupViewValue = "";
            } else {
                $this->updated_at->LevelBreak = false;
            }
        ...
        if ($this->RowType == RowType::TOTAL) {
            ...
            $currentValue = $this->updated_at->groupValue();
            $viewValue = &$this->updated_at->GroupViewValue;
            $viewAttrs = &$this->updated_at->ViewAttrs;
            $cellAttrs = &$this->updated_at->CellAttrs;
            $hrefValue = &$this->updated_at->HrefValue;
            $linkAttrs = &$this->updated_at->LinkAttrs;
            $this->cellRendered($this->updated_at, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fwesane2srch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fwesane2srch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
