 ...
 * Page class
 */
class Crosstab3Crosstab extends Crosstab3
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "Crosstab3Crosstab";
    ...
    public $CurrentPageName = "Crosstab3";
    ...
    public function __construct()
    {
        ...
        // Table object (Crosstab3)
        if (!isset($GLOBALS["Crosstab3"]) || $GLOBALS["Crosstab3"]::class == PROJECT_NAMESPACE . "Crosstab3") {
            $GLOBALS["Crosstab3"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Crosstab3');
        }
    ...
    public function run()
    {
        ...
        if ($year != "") {
            AddFilter($this->SearchWhere, "YEAR(`appealDate`) = " . QuotedValue($year, DataType::STRING, $this->Dbid));
        }
        ...
        $this->appealDate->CssClass = "";
        $this->appealDate->CellCssStyle = "";
    ...
    public function loadGroupRowValues()
    {
        ...
        if ($this->GroupCount < $cnt) {
            $this->rank->setGroupValue(reset($this->GroupRecords[$this->GroupCount]));
        } else {
            $this->rank->setGroupValue("");
        }
    }
    ...
    {
        $this->rank->setDbValue($record['rank']);
        $cntbase = 2;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::SEARCH) { // Search row
            ...
            // rank
            $curVal = strval($this->rank->groupValue());
            if ($curVal != "") {
                $this->rank->GroupViewValue = $this->rank->lookupCacheOption($curVal);
                if ($this->rank->GroupViewValue === null) { // Lookup from database
                    ...
                    if ($ari > 0) { // Lookup values found
                        ...
                        $this->rank->GroupViewValue = $this->rank->displayValue($arwrk);
                    } else {
                        $this->rank->GroupViewValue = $this->rank->groupValue();
                    }
                }
            } else {
                $this->rank->GroupViewValue = null;
            }
            $this->rank->CellCssClass = ($this->RowGroupLevel == 1 ? "ew-rpt-grp-summary-1" : "ew-rpt-grp-field-1");
            ...
            $this->rank->CellCssClass = "ew-rpt-grp-field-1";
            if (!$this->rank->LevelBreak) {
                $this->rank->GroupViewValue = "";
            } else {
                $this->rank->LevelBreak = false;
            }
        ...
        if ($this->RowType == RowType::TOTAL) {
            ...
            $currentValue = $this->rank->groupValue();
            $viewValue = &$this->rank->GroupViewValue;
            $viewAttrs = &$this->rank->ViewAttrs;
            $cellAttrs = &$this->rank->CellAttrs;
            $hrefValue = &$this->rank->HrefValue;
            $linkAttrs = &$this->rank->LinkAttrs;
            $this->cellRendered($this->rank, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
    ...
    protected function getYearSelection()
    {
        ...
        $year = "";
        if (Get("YEAR__appealDate") !== null) {
            $this->YEAR__appealDate->setQueryStringValue(Get("YEAR__appealDate"));
            if (is_numeric($this->YEAR__appealDate->QueryStringValue)) {
                $year = $this->YEAR__appealDate->QueryStringValue;
                $this->resetPager();
            }
        }
        ...
        foreach ($rsyear as $row) {
            if ($row[0] !== null) {
                $this->YEAR__appealDate->DistinctValues[] = $row[0];
            }
        }

        // Restore from session
        if ($year == "" && $this->YEAR__appealDate->AdvancedSearch->issetSession()) {
            $this->YEAR__appealDate->AdvancedSearch->load();
            $year = $this->YEAR__appealDate->AdvancedSearch->SearchValue;
        }

        // Use first record
        if ($year == "" && count($this->YEAR__appealDate->DistinctValues) > 0) {
            $year = $this->YEAR__appealDate->DistinctValues[0];
        }
        $this->YEAR__appealDate->CurrentValue = $year; // Save to CurrentValue
        $this->YEAR__appealDate->AdvancedSearch->SearchValue = $year;
        $this->YEAR__appealDate->AdvancedSearch->save(); // Save to session
        return $year;
    }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fCrosstab3srch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->appealDate->Visible;
    }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fCrosstab3srch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fCrosstab3srch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function showFilterList()
    {
        ...
        // Year Filter
        if (strval($this->YEAR__appealDate->CurrentValue) != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $Language->phrase("Year") . "</span>" . $captionSuffix;
            $filterList .= "<span class=\"ew-filter-value\">" . $this->YEAR__appealDate->CurrentValue . "</span></div>";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fCrosstab3srch");
        }
        ...
        if (strval($this->YEAR__appealDate->CurrentValue) != "") {
            ...
            $filterList .= "\"YEAR__appealDate\":\"" . JsEncode($this->YEAR__appealDate->CurrentValue) . "\"";
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fCrosstab3srch", $filters);
