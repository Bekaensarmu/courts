...
class Judges extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->judgeID = new DbField(
            $this, // Table
            'x_judgeID', // Variable name
            'judgeID', // Name
            '`judgeID`', // Expression
            '`judgeID`', // Basic search expression
            200, // Type
            255, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`judgeID`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->judgeID->Nullable = false; // NOT NULL field
        $this->judgeID->Required = true; // Required field
        $this->judgeID->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        ...
        // courtTyep
        $this->courtTyep = new DbField(
            $this, // Table
            'x_courtTyep', // Variable name
            'courtTyep', // Name
            '`courtTyep`', // Expression
            '`courtTyep`', // Basic search expression
            200, // Type
            255, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`courtTyep`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->courtTyep->InputTextType = "text";
        $this->courtTyep->Nullable = false; // NOT NULL field
        $this->courtTyep->Required = true; // Required field
        $this->courtTyep->setSelectMultiple(false); // Select one
        $this->courtTyep->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->courtTyep->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        $this->courtTyep->Lookup = new Lookup($this->courtTyep, 'courts', false, 'courtID', ["name","","",""], '', '', [], [], [], [], [], [], false, '', '', "`name`");
        $this->courtTyep->SearchOperators = ["=", "<>"];
        $this->Fields['courtTyep'] = &$this->courtTyep;
        ...
        // Emptype
        $this->Emptype = new DbField(
            $this, // Table
            'x_Emptype', // Variable name
            'Emptype', // Name
            '`Emptype`', // Expression
            '`Emptype`', // Basic search expression
            200, // Type
            255, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Emptype`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->Emptype->InputTextType = "text";
        $this->Emptype->Nullable = false; // NOT NULL field
        $this->Emptype->Required = true; // Required field
        $this->Emptype->Lookup = new Lookup($this->Emptype, 'judges', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->Emptype->OptionCount = 2;
        $this->Emptype->SearchOperators = ["=", "<>"];
        $this->Fields['Emptype'] = &$this->Emptype;
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->courtTyep->DbValue = $row['courtTyep'];
        $this->Address->DbValue = $row['Address'];
        $this->state->DbValue = $row['state'];
        $this->Emptype->DbValue = $row['Emptype'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("JudgesList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "JudgesView" => $Language->phrase("View"),
            "JudgesEdit" => $Language->phrase("Edit"),
            "JudgesAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "JudgesView",
            Config("API_ADD_ACTION") => "JudgesAdd",
            Config("API_EDIT_ACTION") => "JudgesEdit",
            Config("API_DELETE_ACTION") => "JudgesDelete",
            Config("API_LIST_ACTION") => "JudgesList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("JudgesView", $parm);
        } else {
            $url = $this->keyUrl("JudgesView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("JudgesEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("JudgesList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("JudgesAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("JudgesList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("JudgesDelete", $parm);
        }
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->courtTyep);
                    $doc->exportCaption($this->Address);
                    $doc->exportCaption($this->state);
                    $doc->exportCaption($this->Emptype);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->courtTyep);
                        $doc->exportField($this->Address);
                        $doc->exportField($this->state);
                        $doc->exportField($this->Emptype);
