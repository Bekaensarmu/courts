 ...
 * Page class
 */
class CaseChargesEdit extends CaseCharges
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CaseChargesEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "CaseChargesEdit";
    ...
    public function setVisibility()
    {
        ...
        $this->crimeDescription->setVisibility();
    ...
    public function run()
    {
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("CaseChargesList"); // Return to list page
                        return;
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("CaseChargesList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "CaseChargesList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "CaseChargesList") {
                            ...
                            $returnUrl = "CaseChargesList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'crimeDescription' first before field var 'x_crimeDescription'
        $val = $CurrentForm->hasValue("crimeDescription") ? $CurrentForm->getValue("crimeDescription") : $CurrentForm->getValue("x_crimeDescription");
        if (!$this->crimeDescription->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->crimeDescription->Visible = false; // Disable update for API request
            } else {
                $this->crimeDescription->setFormValue($val);
            }
        }
        ...
        // Check field name 'ChargeDate' first before field var 'x_ChargeDate'
        $val = $CurrentForm->hasValue("ChargeDate") ? $CurrentForm->getValue("ChargeDate") : $CurrentForm->getValue("x_ChargeDate");
        if (!$this->ChargeDate->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->ChargeDate->Visible = false; // Disable update for API request
            } else {
                $this->ChargeDate->setFormValue($val, true, $validate);
            }
            $this->ChargeDate->CurrentValue = UnFormatDateTime($this->ChargeDate->CurrentValue, $this->ChargeDate->formatPattern());
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->crimeDescription->CurrentValue = $this->crimeDescription->FormValue;
        $this->crimeDate->CurrentValue = $this->crimeDate->FormValue;
        $this->crimeDate->CurrentValue = UnFormatDateTime($this->crimeDate->CurrentValue, $this->crimeDate->formatPattern());
        $this->ChargeDate->CurrentValue = $this->ChargeDate->FormValue;
        $this->ChargeDate->CurrentValue = UnFormatDateTime($this->ChargeDate->CurrentValue, $this->ChargeDate->formatPattern());
    ...
    public function renderRow()
    {
        ...
        // crimeDescription
        $this->crimeDescription->RowCssClass = "row";

        // crimeDate
        $this->crimeDate->RowCssClass = "row";

        // ChargeDate
        $this->ChargeDate->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->deptName->EditValue = HtmlEncode($this->deptName->CurrentValue);
            ...
            $this->mid->EditValue = HtmlEncode($this->mid->CurrentValue);
            ...
            $this->crimeDescription->EditValue = HtmlEncode($this->crimeDescription->CurrentValue);
            ...
            $this->ChargeDate->EditValue = HtmlEncode(FormatDateTime($this->ChargeDate->CurrentValue, $this->ChargeDate->formatPattern()));
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->crimeDescription->Visible && $this->crimeDescription->Required) {
                if (!$this->crimeDescription->IsDetailKey && EmptyValue($this->crimeDescription->FormValue)) {
                    $this->crimeDescription->addErrorMessage(str_replace("%s", $this->crimeDescription->caption(), $this->crimeDescription->RequiredErrorMessage));
                }
            }
            ...
            if ($this->ChargeDate->Visible && $this->ChargeDate->Required) {
                if (!$this->ChargeDate->IsDetailKey && EmptyValue($this->ChargeDate->FormValue)) {
                    $this->ChargeDate->addErrorMessage(str_replace("%s", $this->ChargeDate->caption(), $this->ChargeDate->RequiredErrorMessage));
                }
            }
            if (!CheckDate($this->ChargeDate->FormValue, $this->ChargeDate->formatPattern())) {
                $this->ChargeDate->addErrorMessage($this->ChargeDate->getErrorMessage(false));
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // crimeDescription
        $this->crimeDescription->setDbValueDef($rsnew, $this->crimeDescription->CurrentValue, $this->crimeDescription->ReadOnly);
        ...
        // ChargeDate
        $this->ChargeDate->setDbValueDef($rsnew, UnFormatDateTime($this->ChargeDate->CurrentValue, $this->ChargeDate->formatPattern()), $this->ChargeDate->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['crimeDescription'])) { // crimeDescription
            $this->crimeDescription->CurrentValue = $row['crimeDescription'];
        }
        if (isset($row['crimeDate'])) { // crimeDate
            $this->crimeDate->CurrentValue = $row['crimeDate'];
        }
        if (isset($row['ChargeDate'])) { // ChargeDate
            $this->ChargeDate->CurrentValue = $row['ChargeDate'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("CaseChargesList"), "", $this->TableVar, true);
        $pageId = "edit";
