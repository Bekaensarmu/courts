 ...
 * Page class
 */
class JudgesAdd extends Judges
{
    use MessagesTrait;
    ...
    // CSS class/style
    public $CurrentPageName = "JudgesAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("JudgesList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "JudgesList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "JudgesView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "JudgesList") {
                            ...
                            $returnUrl = "JudgesList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'courtTyep' first before field var 'x_courtTyep'
        $val = $CurrentForm->hasValue("courtTyep") ? $CurrentForm->getValue("courtTyep") : $CurrentForm->getValue("x_courtTyep");
        if (!$this->courtTyep->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->courtTyep->Visible = false; // Disable update for API request
            } else {
                $this->courtTyep->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->courtTyep->CurrentValue = $this->courtTyep->FormValue;
    ...
    public function renderRow()
    {
        ...
        // courtTyep
        $this->courtTyep->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->judgeID->Raw) {
                $this->judgeID->CurrentValue = HtmlDecode($this->judgeID->CurrentValue);
            }
            $this->judgeID->EditValue = HtmlEncode($this->judgeID->CurrentValue);
            ...
            // courtTyep
            $this->courtTyep->setupEditAttributes();
            $curVal = trim(strval($this->courtTyep->CurrentValue));
            if ($curVal != "") {
                $this->courtTyep->ViewValue = $this->courtTyep->lookupCacheOption($curVal);
            } else {
                $this->courtTyep->ViewValue = $this->courtTyep->Lookup !== null && is_array($this->courtTyep->lookupOptions()) && count($this->courtTyep->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->courtTyep->ViewValue !== null) { // Load from cache
                $this->courtTyep->EditValue = array_values($this->courtTyep->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->courtTyep->Lookup->getTable()->Fields["courtID"]->searchExpression(), "=", $this->courtTyep->CurrentValue, $this->courtTyep->Lookup->getTable()->Fields["courtID"]->searchDataType(), "");
                }
                $sqlWrk = $this->courtTyep->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCache($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->courtTyep->EditValue = $arwrk;
            }
            $this->courtTyep->PlaceHolder = RemoveHtml($this->courtTyep->caption());
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->judgeID->Visible && $this->judgeID->Required) {
            ...
            if ($this->courtTyep->Visible && $this->courtTyep->Required) {
                if (!$this->courtTyep->IsDetailKey && EmptyValue($this->courtTyep->FormValue)) {
                    $this->courtTyep->addErrorMessage(str_replace("%s", $this->courtTyep->caption(), $this->courtTyep->RequiredErrorMessage));
                }
            }
    ...
    protected function getAddRow()
    {
        ...
        // judgeID
        $this->judgeID->setDbValueDef($rsnew, $this->judgeID->CurrentValue, false);

        // name
        $this->name->setDbValueDef($rsnew, $this->name->CurrentValue, false);

        // courtTyep
        $this->courtTyep->setDbValueDef($rsnew, $this->courtTyep->CurrentValue, false);
        ...
        // Emptype
        $this->Emptype->setDbValueDef($rsnew, $this->Emptype->CurrentValue, false);
    ...
    {
        if (isset($row['judgeID'])) { // judgeID
            $this->judgeID->setFormValue($row['judgeID']);
        }
        if (isset($row['name'])) { // name
            $this->name->setFormValue($row['name']);
        }
        if (isset($row['courtTyep'])) { // courtTyep
            $this->courtTyep->setFormValue($row['courtTyep']);
        }
        ...
        if (isset($row['Emptype'])) { // Emptype
            $this->Emptype->setFormValue($row['Emptype']);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("JudgesList"), "", $this->TableVar, true);
