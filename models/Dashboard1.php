...
class Dashboard1 extends ReportTable
{
    ...
    public function renderItem($view, $id)
    {
        ...
        try {
            if ($id == 1) {
                $Page = new Crosstab1Crosstab();
                $Page->Export = $this->Export;
                $Page->UseAjaxActions = true; // Use Ajax actions for pager/sort
                $Page->run();
                $content = $view->fetch("Crosstab1Crosstab.php", $GLOBALS);
            }
            if ($id == 2) {
                $Page = new Crosstab2Crosstab();
                $Page->Export = $this->Export;
                $Page->UseAjaxActions = true; // Use Ajax actions for pager/sort
                $Page->run();
                $content = $view->fetch("Crosstab2Crosstab.php", $GLOBALS);
            }
            if ($id == 3) {
                $Page = new Crosstab3Crosstab();
                $Page->Export = $this->Export;
                $Page->UseAjaxActions = true; // Use Ajax actions for pager/sort
                $Page->run();
                $content = $view->fetch("Crosstab3Crosstab.php", $GLOBALS);
            }
        } catch (\Exception $e) {
            $GLOBALS["Page"] = $this;
        } finally {
