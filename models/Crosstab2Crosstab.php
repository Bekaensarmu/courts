 ...
 * Page class
 */
class Crosstab2Crosstab extends Crosstab2
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "Crosstab2Crosstab";
    ...
    public $CurrentPageName = "Crosstab2";
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["Crosstab2"]) || $GLOBALS["Crosstab2"]::class == PROJECT_NAMESPACE . "Crosstab2") {
    ...
    public function run()
    {
        ...
        if ($year != "") {
            AddFilter($this->SearchWhere, "YEAR(`decisionDate`) = " . QuotedValue($year, DataType::STRING, $this->Dbid));
        }
        ...
        $this->decisionDate->CssClass = "";
        $this->decisionDate->CellCssStyle = "";
    ...
    public function loadGroupRowValues()
    {
        ...
        if ($this->GroupCount < $cnt) {
            $this->ez->setGroupValue(reset($this->GroupRecords[$this->GroupCount]));
        } else {
            $this->ez->setGroupValue("");
        }
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::SEARCH) { // Search row
            ...
            // ez
            $curVal = strval($this->ez->groupValue());
            if ($curVal != "") {
                $this->ez->GroupViewValue = $this->ez->lookupCacheOption($curVal);
                if ($this->ez->GroupViewValue === null) { // Lookup from database
                    ...
                    if ($ari > 0) { // Lookup values found
                        ...
                        $this->ez->GroupViewValue = $this->ez->displayValue($arwrk);
                    } else {
                        $this->ez->GroupViewValue = $this->ez->groupValue();
                    }
                }
            } else {
                $this->ez->GroupViewValue = null;
            }
            $this->ez->CellCssClass = ($this->RowGroupLevel == 1 ? "ew-rpt-grp-summary-1" : "ew-rpt-grp-field-1");
            ...
            $this->ez->CellCssClass = "ew-rpt-grp-field-1";
            if (!$this->ez->LevelBreak) {
                $this->ez->GroupViewValue = "";
            } else {
                $this->ez->LevelBreak = false;
            }
        ...
        if ($this->RowType == RowType::TOTAL) {
            ...
            $currentValue = $this->ez->groupValue();
            $viewValue = &$this->ez->GroupViewValue;
    ...
    protected function getYearSelection()
    {
        ...
        $year = "";
        if (Get("YEAR__decisionDate") !== null) {
            $this->YEAR__decisionDate->setQueryStringValue(Get("YEAR__decisionDate"));
            if (is_numeric($this->YEAR__decisionDate->QueryStringValue)) {
                $year = $this->YEAR__decisionDate->QueryStringValue;
                $this->resetPager();
            }
        }
        ...
        foreach ($rsyear as $row) {
            if ($row[0] !== null) {
                $this->YEAR__decisionDate->DistinctValues[] = $row[0];
            }
        }

        // Restore from session
        if ($year == "" && $this->YEAR__decisionDate->AdvancedSearch->issetSession()) {
            $this->YEAR__decisionDate->AdvancedSearch->load();
            $year = $this->YEAR__decisionDate->AdvancedSearch->SearchValue;
        }

        // Use first record
        if ($year == "" && count($this->YEAR__decisionDate->DistinctValues) > 0) {
            $year = $this->YEAR__decisionDate->DistinctValues[0];
        }
        $this->YEAR__decisionDate->CurrentValue = $year; // Save to CurrentValue
        $this->YEAR__decisionDate->AdvancedSearch->SearchValue = $year;
        $this->YEAR__decisionDate->AdvancedSearch->save(); // Save to session
        return $year;
    }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fCrosstab2srch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fCrosstab2srch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fCrosstab2srch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function showFilterList()
    {
        ...
        // Year Filter
        if (strval($this->YEAR__decisionDate->CurrentValue) != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $Language->phrase("Year") . "</span>" . $captionSuffix;
            $filterList .= "<span class=\"ew-filter-value\">" . $this->YEAR__decisionDate->CurrentValue . "</span></div>";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fCrosstab2srch");
        }
        ...
        if (strval($this->YEAR__decisionDate->CurrentValue) != "") {
            ...
            $filterList .= "\"YEAR__decisionDate\":\"" . JsEncode($this->YEAR__decisionDate->CurrentValue) . "\"";
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fCrosstab2srch", $filters);
