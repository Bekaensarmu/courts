 ...
 * Page class
 */
class Report3Summary extends Report3
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "Report3Summary";
    ...
    public $CurrentPageName = "Report3";
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["Report3"]) || $GLOBALS["Report3"]::class == PROJECT_NAMESPACE . "Report3") {
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Report3');
        }
    ...
    public function loadGroupRowValues()
    {
        ...
        if ($this->GroupCount < $cnt) {
            $this->fileNumber->setGroupValue(reset($this->GroupRecords[$this->GroupCount]));
        } else {
            $this->fileNumber->setGroupValue("");
        }
    }
    ...
    public function loadRowValues($record)
    {
        ...
        $this->fileNumber->setDbValue(GroupValue($this->fileNumber, $record['fileNumber']));
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::SEARCH) {
            ...
            if ($this->RowTotalType == RowSummary::GROUP) {
                $this->RowAttrs["data-group"] = $this->fileNumber->groupValue(); // Set up group attribute
            }

            // fileNumber
            $this->fileNumber->GroupViewValue = $this->fileNumber->groupValue();
            $this->fileNumber->CellCssClass = ($this->RowGroupLevel == 1 ? "ew-rpt-grp-summary-1" : "ew-rpt-grp-field-1");
            $this->fileNumber->GroupViewValue = DisplayGroupValue($this->fileNumber, $this->fileNumber->GroupViewValue);
            ...
            // fileNumber
            $this->fileNumber->GroupViewValue = $this->fileNumber->groupValue();
            $this->fileNumber->CellCssClass = "ew-rpt-grp-field-1";
            $this->fileNumber->GroupViewValue = DisplayGroupValue($this->fileNumber, $this->fileNumber->GroupViewValue);
            if (!$this->fileNumber->LevelBreak) {
                $this->fileNumber->GroupViewValue = "";
            } else {
                $this->fileNumber->LevelBreak = false;
            }
        ...
        if ($this->RowType == RowType::TOTAL) {
            // fileNumber
            $currentValue = $this->fileNumber->GroupViewValue;
            $viewValue = &$this->fileNumber->GroupViewValue;
            ...
            // fileNumber
            $currentValue = $this->fileNumber->groupValue();
            $viewValue = &$this->fileNumber->GroupViewValue;
            $viewAttrs = &$this->fileNumber->ViewAttrs;
            $cellAttrs = &$this->fileNumber->CellAttrs;
            $hrefValue = &$this->fileNumber->HrefValue;
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fReport3srch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->fileNumber->Visible;
    }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fReport3srch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fReport3srch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fReport3srch");
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fReport3srch", $filters);
