...
class Appointments extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // case_hear_id
        $this->case_hear_id = new DbField(
            $this, // Table
            'x_case_hear_id', // Variable name
            'case_hear_id', // Name
            '`case_hear_id`', // Expression
            '`case_hear_id`', // Basic search expression
            21, // Type
            20, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`case_hear_id`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->case_hear_id->InputTextType = "text";
        $this->case_hear_id->Raw = true;
        $this->case_hear_id->Nullable = false; // NOT NULL field
        $this->case_hear_id->Required = true; // Required field
        $this->case_hear_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->case_hear_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['case_hear_id'] = &$this->case_hear_id;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "appointments";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->case_hear_id->DbValue = $row['case_hear_id'];
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "AppointmentsView",
            Config("API_ADD_ACTION") => "AppointmentsAdd",
            Config("API_EDIT_ACTION") => "AppointmentsEdit",
            Config("API_DELETE_ACTION") => "AppointmentsDelete",
            Config("API_LIST_ACTION") => "AppointmentsList",
            default => ""
        };
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->case_hear_id);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->case_hear_id);
