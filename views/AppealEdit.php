...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fappealedit" id="fappealedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<span id="el_appeal_AppealID">
    <select
        ...
        data-select2-id="fappealedit_x_AppealID"
        <?php } ?>
        data-table="appeal"
...
<?php if (!$Page->AppealID->IsNativeSelect) { ?>
<script>
loadjs.ready("fappealedit", function() {
    var options = { name: "x_AppealID", selectId: "fappealedit_x_AppealID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fappealedit.lists.AppealID?.lookupOptions.length) {
        options.data = { id: "x_AppealID", form: "fappealedit" };
    } else {
        options.ajax = { id: "x_AppealID", form: "fappealedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fappealedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fappealedit", "x_appealDate", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_appeal_mid">
    <select
        ...
        data-select2-id="fappealedit_x_mid"
        <?php } ?>
        data-table="appeal"
        data-field="x_mid"
...
<script>
loadjs.ready("fappealedit", function() {
    var options = { name: "x_mid", selectId: "fappealedit_x_mid" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fappealedit.lists.mid?.lookupOptions.length) {
        options.data = { id: "x_mid", form: "fappealedit" };
    } else {
        options.ajax = { id: "x_mid", form: "fappealedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_appeal_rank">
    <select
        ...
        data-select2-id="fappealedit_x_rank"
        <?php } ?>
        data-table="appeal"
        data-field="x_rank"
...
<script>
loadjs.ready("fappealedit", function() {
    var options = { name: "x_rank", selectId: "fappealedit_x_rank" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fappealedit.lists.rank?.lookupOptions.length) {
        options.data = { id: "x_rank", form: "fappealedit" };
    } else {
        options.ajax = { id: "x_rank", form: "fappealedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_appeal_deptName">
    <select
        ...
        data-select2-id="fappealedit_x_deptName"
        <?php } ?>
        data-table="appeal"
...
<script>
loadjs.ready("fappealedit", function() {
    var options = { name: "x_deptName", selectId: "fappealedit_x_deptName" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fappealedit.lists.deptName?.lookupOptions.length) {
        options.data = { id: "x_deptName", form: "fappealedit" };
    } else {
        options.ajax = { id: "x_deptName", form: "fappealedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_appeal_midib">
    <select
        ...
        data-select2-id="fappealedit_x_midib"
        <?php } ?>
        data-table="appeal"
        data-field="x_midib"
...
<script>
loadjs.ready("fappealedit", function() {
    var options = { name: "x_midib", selectId: "fappealedit_x_midib" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fappealedit.lists.midib?.lookupOptions.length) {
        options.data = { id: "x_midib", form: "fappealedit" };
    } else {
        options.ajax = { id: "x_midib", form: "fappealedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_appeal_appealask">
    <select
        ...
        data-select2-id="fappealedit_x_appealask"
        <?php } ?>
        data-table="appeal"
...
<script>
loadjs.ready("fappealedit", function() {
    var options = { name: "x_appealask", selectId: "fappealedit_x_appealask" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fappealedit.lists.appealask?.lookupOptions.length) {
        options.data = { id: "x_appealask", form: "fappealedit" };
    } else {
        options.ajax = { id: "x_appealask", form: "fappealedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_appeal_AppealDescription">
    <select
        ...
        data-select2-id="fappealedit_x_AppealDescription"
        <?php } ?>
        data-table="appeal"
...
<script>
loadjs.ready("fappealedit", function() {
    var options = { name: "x_AppealDescription", selectId: "fappealedit_x_AppealDescription" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fappealedit.lists.AppealDescription?.lookupOptions.length) {
        options.data = { id: "x_AppealDescription", form: "fappealedit" };
    } else {
        options.ajax = { id: "x_AppealDescription", form: "fappealedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_appeal_appealDecision">
    <select
        ...
        data-select2-id="fappealedit_x_appealDecision"
        <?php } ?>
        data-table="appeal"
...
<script>
loadjs.ready("fappealedit", function() {
    var options = { name: "x_appealDecision", selectId: "fappealedit_x_appealDecision" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fappealedit.lists.appealDecision?.lookupOptions.length) {
        options.data = { id: "x_appealDecision", form: "fappealedit" };
    } else {
        options.ajax = { id: "x_appealDecision", form: "fappealedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
loadjs.ready(["fappealedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fappealedit", "x_created_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fappealedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fappealedit", "x_crimeDate", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php } ?>
    ...
    <input type="hidden" data-table="appeal" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fappealedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fappealedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
