...
ew.deepAssign(ew.vars, { tables: { Crosstab1: currentTable } });
...
<?php if (!$Page->isExport() && !$Page->DrillDown && !$DashboardReport) { ?>
<form name="fCrosstab1srch" id="fCrosstab1srch" class="ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>" novalidate autocomplete="off">
<div id="fCrosstab1srch_search_panel" class="mb-2 mb-sm-0 <?= $Page->SearchPanelClass ?>"><!-- .ew-search-panel -->
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { Crosstab1: currentTable } });
var currentPageID = ew.PAGE_ID = "crosstab";
var currentForm;
var fCrosstab1srch, currentSearchForm, currentAdvancedSearchForm;
...
<?php } ?>
<!-- Report grid (begin) -->
<div id="gmp_Crosstab1" class="card-body ew-grid-middle-panel <?= $Page->TableContainerClass ?>">
...
<?php } ?>
    ...
    <tr class="ew-table-header">
<?php if ($Page->RANKe->Visible) { ?>
    <td data-field="RANKe"><div><?= $Page->renderFieldHeader($Page->RANKe) ?></div></td>
<?php } ?>
...
?>
<!-- Dynamic columns end -->
        <td class="ew-table-header"<?= $Page->created_at->cellAttributes() ?>><div<?= $Page->created_at->viewAttributes() ?>><?= $Page->renderSummaryCaptions() ?></div></td>
    </tr>
</thead>
<tbody>
<?php
...
<?php
    ...
    // Build detail SQL
    $where = DetailFilterSql($Page->RANKe, $Page->getSqlFirstGroupField(), $Page->RANKe->groupValue(), $Page->Dbid);
    ...
    // Load detail records
    $Page->RANKe->Records = &$Page->DetailRecords;
    $Page->RANKe->LevelBreak = true; // Set field level break
        foreach ($Page->RANKe->Records as $record) {
            $Page->RecordCount++;
            $Page->RecordIndex++;
...
<?php if ($Page->RANKe->Visible) { ?>
        <!-- RANKe -->
        <td data-field="RANKe"<?= $Page->RANKe->cellAttributes() ?>><span<?= $Page->RANKe->viewAttributes() ?>><?= $Page->RANKe->GroupViewValue ?></span></td>
<?php } ?>
<!-- Dynamic columns begin -->
<?php
