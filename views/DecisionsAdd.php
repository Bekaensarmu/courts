...
$Page->showMessage();
?>
<form name="fdecisionsadd" id="fdecisionsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<script>
loadjs.ready(["fdecisionsadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdecisionsadd", "x_decisionDate", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_decisions_Decisiontype">
    <select
        ...
        data-select2-id="fdecisionsadd_x_Decisiontype"
        <?php } ?>
...
<?php if (!$Page->Decisiontype->IsNativeSelect) { ?>
<script>
loadjs.ready("fdecisionsadd", function() {
    var options = { name: "x_Decisiontype", selectId: "fdecisionsadd_x_Decisiontype" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdecisionsadd.lists.Decisiontype?.lookupOptions.length) {
        options.data = { id: "x_Decisiontype", form: "fdecisionsadd" };
    } else {
        options.ajax = { id: "x_Decisiontype", form: "fdecisionsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_decisions_kistype">
    <select
        ...
        data-select2-id="fdecisionsadd_x_kistype"
        <?php } ?>
        data-table="decisions"
...
<script>
loadjs.ready("fdecisionsadd", function() {
    var options = { name: "x_kistype", selectId: "fdecisionsadd_x_kistype" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdecisionsadd.lists.kistype?.lookupOptions.length) {
        options.data = { id: "x_kistype", form: "fdecisionsadd" };
    } else {
        options.ajax = { id: "x_kistype", form: "fdecisionsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_decisions_ez">
    <select
        ...
        data-select2-id="fdecisionsadd_x_ez"
        <?php } ?>
        data-table="decisions"
        data-field="x_ez"
...
<script>
loadjs.ready("fdecisionsadd", function() {
    var options = { name: "x_ez", selectId: "fdecisionsadd_x_ez" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdecisionsadd.lists.ez?.lookupOptions.length) {
        options.data = { id: "x_ez", form: "fdecisionsadd" };
    } else {
        options.ajax = { id: "x_ez", form: "fdecisionsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_decisions_chilot">
    <select
        ...
        data-select2-id="fdecisionsadd_x_chilot"
        <?php } ?>
        data-table="decisions"
        data-field="x_chilot"
...
<script>
loadjs.ready("fdecisionsadd", function() {
    var options = { name: "x_chilot", selectId: "fdecisionsadd_x_chilot" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdecisionsadd.lists.chilot?.lookupOptions.length) {
        options.data = { id: "x_chilot", form: "fdecisionsadd" };
    } else {
        options.ajax = { id: "x_chilot", form: "fdecisionsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
loadjs.ready(["fdecisionsadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdecisionsadd", "x_created_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fdecisionsadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fdecisionsadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
