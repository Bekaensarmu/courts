...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fjudgesedit" id="fjudgesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<span id="el_judges_courtTyep">
    <select
        ...
        data-select2-id="fjudgesedit_x_courtTyep"
        <?php } ?>
        data-table="judges"
...
<?php if (!$Page->courtTyep->IsNativeSelect) { ?>
<script>
loadjs.ready("fjudgesedit", function() {
    var options = { name: "x_courtTyep", selectId: "fjudgesedit_x_courtTyep" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fjudgesedit.lists.courtTyep?.lookupOptions.length) {
        options.data = { id: "x_courtTyep", form: "fjudgesedit" };
    } else {
        options.ajax = { id: "x_courtTyep", form: "fjudgesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fjudgesedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fjudgesedit", "x_created_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php } ?>
    ...
    <input type="hidden" data-table="judges" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fjudgesedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fjudgesedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
