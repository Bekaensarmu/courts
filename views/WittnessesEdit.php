...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fwittnessesedit" id="fwittnessesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fwittnessesedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["id", [fields.id.visible && fields.id.required ? ew.Validators.required(fields.id.caption) : null, ew.Validators.integer], fields.id.isInvalid],
            ["wittnessID", [fields.wittnessID.visible && fields.wittnessID.required ? ew.Validators.required(fields.wittnessID.caption) : null], fields.wittnessID.isInvalid],
...
<input type="hidden" data-table="wittnesses" data-field="x_id" data-hidden="1" data-old name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
<?php if ($Page->wittnessID->Visible) { // wittnessID ?>
    <div id="r_wittnessID"<?= $Page->wittnessID->rowAttributes() ?>>
        <label id="elh_wittnesses_wittnessID" for="x_wittnessID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->wittnessID->caption() ?><?= $Page->wittnessID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->wittnessID->cellAttributes() ?>>
<span id="el_wittnesses_wittnessID">
<input type="<?= $Page->wittnessID->getInputTextType() ?>" name="x_wittnessID" id="x_wittnessID" data-table="wittnesses" data-field="x_wittnessID" value="<?= $Page->wittnessID->EditValue ?>" size="30" maxlength="255" placeholder="<?= HtmlEncode($Page->wittnessID->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->wittnessID->formatPattern()) ?>"<?= $Page->wittnessID->editAttributes() ?> aria-describedby="x_wittnessID_help">
<?= $Page->wittnessID->getCustomMessage() ?>
<div class="invalid-feedback"><?= $Page->wittnessID->getErrorMessage() ?></div>
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fwittnessesedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fwittnessesedit", "x_created_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fwittnessesedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fwittnessesedit", "x_updated_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fwittnessesedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fwittnessesedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
