...
// Page object
$Crosstab2Crosstab = &$Page;
?>
<?php if (!$Page->isExport() && !$Page->DrillDown && !$DashboardReport) { ?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { Crosstab2: currentTable } });
...
<?php if (!$Page->isExport() && !$Page->DrillDown && !$DashboardReport) { ?>
<form name="fCrosstab2srch" id="fCrosstab2srch" class="ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>" novalidate autocomplete="off">
<div id="fCrosstab2srch_search_panel" class="mb-2 mb-sm-0 <?= $Page->SearchPanelClass ?>"><!-- .ew-search-panel -->
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { Crosstab2: currentTable } });
var currentPageID = ew.PAGE_ID = "crosstab";
var currentForm;
var fCrosstab2srch, currentSearchForm, currentAdvancedSearchForm;
...
?>
    <div id="xs_decisionDate" class="col-sm-auto d-sm-flex align-items-start mb-3 px-0 pe-sm-2<?= $Page->decisionDate->UseFilter ? " ew-filter-field" : "" ?>">
        <label for="YEAR__decisionDate" class="ew-search-caption ew-label"><?= $Language->phrase("Year") ?></label>
        <div class="ew-search-field">
            <select id="YEAR__decisionDate" class="form-select" name="YEAR__decisionDate">
            <?php
                // Set up year selector
                if (is_array($Page->YEAR__decisionDate->DistinctValues)) {
                    $yearCount = count($Page->YEAR__decisionDate->DistinctValues);
                    for ($yi = 0; $yi < $yearCount; $yi++) {
                        $yearValue = $Page->YEAR__decisionDate->DistinctValues[$yi];
                        $yearSelected = SameString($yearValue, $Page->YEAR__decisionDate->CurrentValue) ? " selected" : "";
            ?>
...
<?php } ?>
<!-- Report grid (begin) -->
<div id="gmp_Crosstab2" class="card-body ew-grid-middle-panel <?= $Page->TableContainerClass ?>">
...
<?php } ?>
        <td class="ew-rpt-col-header" colspan="<?= @$Page->ColumnSpan ?>">
            <div class="ew-table-header-btn">
                <span class="ew-table-header-caption"><?= $Page->decisionDate->caption() ?></span>
            </div>
        </td>
    </tr>
    <tr class="ew-table-header">
<?php if ($Page->ez->Visible) { ?>
    <td data-field="ez"><div><?= $Page->renderFieldHeader($Page->ez) ?></div></td>
<?php } ?>
...
?>
        <td class="ew-table-header"<?= $Page->decisionDate->cellAttributes() ?>><div<?= $Page->decisionDate->viewAttributes() ?>><?= $Page->SummaryViewValues[$iy-1]; ?></div></td>
<?php
        }
    }
?>
<!-- Dynamic columns end -->
        <td class="ew-table-header"<?= $Page->decisionDate->cellAttributes() ?>><div<?= $Page->decisionDate->viewAttributes() ?>><?= $Page->renderSummaryCaptions() ?></div></td>
    </tr>
</thead>
<tbody>
<?php
...
<?php
    ...
    // Build detail SQL
    $where = DetailFilterSql($Page->ez, $Page->getSqlFirstGroupField(), $Page->ez->groupValue(), $Page->Dbid);
    ...
    // Load detail records
    $Page->ez->Records = &$Page->DetailRecords;
    $Page->ez->LevelBreak = true; // Set field level break
        foreach ($Page->ez->Records as $record) {
            $Page->RecordCount++;
            $Page->RecordIndex++;
...
<?php if ($Page->ez->Visible) { ?>
        <!-- ez -->
        <td data-field="ez"<?= $Page->ez->cellAttributes() ?>><span<?= $Page->ez->viewAttributes() ?>><?= $Page->ez->GroupViewValue ?></span></td>
<?php } ?>
<!-- Dynamic columns begin -->
<?php
