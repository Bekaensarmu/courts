...
<input type="hidden" data-table="users" data-field="x_id" data-hidden="1" data-old name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<?php if (SameString($Page->_email->CurrentValue, CurrentUserID())) { ?>
    ...
    <input type="hidden" data-table="users" data-field="x__email" data-hidden="1" name="x__email" id="x__email" value="<?= HtmlEncode($Page->_email->CurrentValue) ?>">
<?php } else { ?>
<span id="el_users__email">
    <select
        ...
        data-select2-id="fusersedit_x__email"
        <?php } ?>
        data-table="users"
        data-field="x__email"
...
loadjs.ready("fusersedit", function() {
    var options = { name: "x__email", selectId: "fusersedit_x__email" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fusersedit.lists._email?.lookupOptions.length) {
        options.data = { id: "x__email", form: "fusersedit" };
    } else {
        options.ajax = { id: "x__email", form: "fusersedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_users_current_team_id">
    <select
        ...
        data-select2-id="fusersedit_x_current_team_id"
        <?php } ?>
        data-table="users"
...
<script>
loadjs.ready("fusersedit", function() {
    var options = { name: "x_current_team_id", selectId: "fusersedit_x_current_team_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fusersedit.lists.current_team_id?.lookupOptions.length) {
        options.data = { id: "x_current_team_id", form: "fusersedit" };
    } else {
        options.ajax = { id: "x_current_team_id", form: "fusersedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
